...
router.get("/dashboard", isLoggedIn, async (req, res) => {
    try {
        const user = await User.findById(req.user.id).select("-password").populate("village");
        if (!user) {
...
router.get("/projects", isLoggedIn, async (req, res) => {
    try {
        ...
        if (!user || !user.village) {
            return res.status(400).json({ message: "User has no assigned village jurisdiction" });
        }
        // Fetch all projects belonging to the user's village
        const projects = await Project.find({ village: user.village }).populate("village", "name district state registeredCitizensCount");
...
router.post("/tickets", isLoggedIn, async (req, res) => {
    try {
        ...
        if (!projectId || !title || !description) {
            return res.status(400).json({ message: "Project, title, and grievance description are required" });
        }
        ...
        res.status(500).json({ message: "Server error filing dispute" });
    }
});

// 5. PUT upvote dispute & execute 50% Automatic Halt protocol
router.put("/tickets/:ticketId/upvote", isLoggedIn, async (req, res) => {
    try {
        ...
        // Toggle Upvote logic
        const hasVoted = ticket.upvotes.some(id => id.toString() === userId.toString());
        if (hasVoted) {
        ...
        if (project) {
            ...
            // If upvotes on this active dispute exceed 50% of registered citizens, halt project
            if ((totalUpvotes / totalCitizens) > 0.5 && ticket.status !== "Resolved") {
        ...
        res.status(200).json({
            message: hasVoted ? "Upvote removed" : "Upvote recorded on ledger",
            ticket: {
                id: ticket._id,
        ...
        res.status(500).json({ message: "Server error toggling vote" });
    }
});
